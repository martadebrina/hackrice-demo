...
async def auth_user(req: Request):
    ...
    # Debug: log what name/email were resolved for this token
    try:
        print(f"[debug] auth_user resolved: sub={payload.get('sub')!r}, name={name!r}, email={email!r}")
    except Exception:
        # Ensure debug logging never breaks auth flow
        pass
...
# --- List open/accepted/completed requests ---
@app.get("/requests")
async def list_requests(status: str = "open", mine: int = 0, user=Depends(auth_user)):
    q = {"status": status}
    if mine:
    ...
    for d in docs:
        ...
        # 👇 add these two
        if isinstance(d.get("scheduledBy"), ObjectId):
            d["scheduledBy"] = str(d["scheduledBy"])
        if isinstance(d.get("scheduledAt"), datetime):
            d["scheduledAt"] = d["scheduledAt"].isoformat().replace("+00:00", "Z")
    return docs
...
def _is_participant(r, uid):
    return r.get("studentId") == uid or r.get("tutorId") == uid

def _utcnow():
    return datetime.now(timezone.utc)

@app.post("/requests/{rid}/schedule")
async def schedule_request(rid: str, payload: dict, user=Depends(auth_user)):
    """
    Propose or update a schedule for an accepted request.
    Body: { scheduledAtISO, durationMin, scheduledTz }
    """
    me = _me(db, user.get("sub"))
    r = _get_req(rid)
    if r.get("status") != "accepted":
        raise HTTPException(400, "Only accepted requests can be scheduled")
    if not _is_participant(r, me["_id"]):
        raise HTTPException(403, "Only participants can schedule")

    scheduled_at_iso: str = (payload.get("scheduledAtISO") or "").strip()
    if not scheduled_at_iso:
        raise HTTPException(400, "scheduledAtISO required")
    try:
        # Expect an ISO string with Z (UTC). Example: "2025-09-20T19:30:00Z"
        sched_dt = datetime.fromisoformat(scheduled_at_iso.replace("Z","+00:00"))
    except Exception:
        raise HTTPException(400, "Invalid scheduledAtISO")

    if sched_dt < _utcnow():
        raise HTTPException(400, "Scheduled time must be in the future")

    duration = int(payload.get("durationMin") or 60)
    if duration <= 0 or duration > 240:
        raise HTTPException(400, "durationMin must be 1–240")

    tz = (payload.get("scheduledTz") or "UTC").strip()
    ...
    await broadcast({"type": "request:scheduled", "rid": rid, "state": "proposed"})
    return {"ok": True}

@app.post("/requests/{rid}/schedule/confirm")
async def schedule_confirm(rid: str, user=Depends(auth_user)):
    me = _me(db, user.get("sub"))
    r = _get_req(rid)
    if r.get("status") != "accepted":
        raise HTTPException(400, "Only accepted requests can be scheduled")
    if not _is_participant(r, me["_id"]):
        raise HTTPException(403, "Only participants can confirm")
    if r.get("scheduleStatus") != "proposed":
        raise HTTPException(400, "Nothing to confirm")

    # only the OTHER participant can confirm
    if r.get("scheduledBy") == me["_id"]:
        raise HTTPException(403, "The other participant must confirm")

    db.requests.update_one(
        {"_id": r["_id"]},
        {"$set": {"scheduleStatus": "confirmed"}}
    )
    await broadcast({"type": "request:scheduled", "rid": rid, "state": "confirmed"})
    return {"ok": True}

@app.post("/requests/{rid}/schedule/cancel")
async def schedule_cancel(rid: str, user=Depends(auth_user)):
    me = _me(db, user.get("sub"))
    r = _get_req(rid)
    if r.get("status") != "accepted":
        raise HTTPException(400, "Only accepted requests can be scheduled")
    if not _is_participant(r, me["_id"]):
        raise HTTPException(403, "Only participants can cancel")
    if r.get("scheduleStatus") not in ("proposed", "confirmed"):
    ...
    await broadcast({"type": "request:scheduled", "rid": rid, "state": "cancelled"})
    return {"ok": True}