...
async def complete_request(rid: str, user=Depends(auth_user)):
    ...
    if r["studentId"] == r["tutorId"]:
        # DEMO MODE: if the same user is both student and tutor, show a visible change.
        # Subtract points from the user (no matching credit).
    ...
    updated = db.requests.find_one_and_update(
        {"_id": r["_id"]},
