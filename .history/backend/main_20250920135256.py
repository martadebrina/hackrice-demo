...
async def get_userinfo_from_auth0(access_token: str):
    ...
    async with httpx.AsyncClient(timeout=5) as s:
        ...
        r.raise_for_status()
        return r.json()  # contains email, name, etc.
...
async def auth_user(req: Request):
    ...
    if not email or not name:
        try:
            ...
            # overwrite/add
            payload["email"] = ui.get("email", email)
            payload["name"] = ui.get("name", name or ui.get("nickname"))
        except Exception:
            # if userinfo fails, keep going without email (your /me will still 400)
            pass
