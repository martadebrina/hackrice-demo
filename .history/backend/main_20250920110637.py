...
app.add_middleware(
    CORSMiddleware,
    # Allow common local development origins explicitly. Using "*" together with
    # allow_credentials=True can cause the browser to reject the response, so
    # list the exact origins the frontend dev server might use.
    allow_origins=[
        "http://localhost:5173",
        "http://127.0.0.1:5173",
        "http://localhost:3000",
