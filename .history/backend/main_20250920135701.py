...
# Debug helper (temporary): return received headers and auth header details.
@app.get("/debug-headers")
async def debug_headers(request: Request):
    # Return a subset of headers to avoid leaking sensitive data in normal logs
    headers = {k: v for k, v in request.headers.items() if k.lower() in ["authorization", "host", "origin", "content-type"]}
