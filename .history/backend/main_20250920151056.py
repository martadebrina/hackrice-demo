...
async def me(user=Depends(auth_user)):
    ...
    # 2) If not found, try to find old email-based doc and migrate it
    if not u:
        ...
        if email:
            ...
            if old:
                db.users.update_one(
                    ...
                    {"$set": {
                        "auth0Sub": sub,
                        "email": email,               # keep fresh email if changed
                        "name": user.get("name"),
                    }}
                )
                u = db.users.find_one({"_id": old["_id"]})

    # 3) Still not found? Create fresh
    if not u:
        u = {
            "auth0Sub": sub,
            "email": user.get("email"),
...
async def accept_request(rid: str, user=Depends(auth_user)):
    sub = user.get("sub")
    tutor = db.users.find_one({"auth0Sub": sub})
    if not tutor:
        raise HTTPException(400, "User not found")
...
# --- Complete a request (safe transfer that returns fresh balances) ---
...
async def complete_request(rid: str, user=Depends(auth_user)):
    ...
    # Fetch participants by stored ObjectIds
    ...
    # Coerce to ints safely (handles if stored as strings)
    try:
        pts = int(r.get("pointsOffered", 0))
        s_pts = int(student.get("points", 0))
        t_pts = int(tutor.get("points", 0))
    except Exception:
        raise HTTPException(500, "Corrupt points data")
    ...
    # Write explicitly with $set to avoid $inc type issues
    ...
    # Who is calling? (so we can return the exact header number)
    caller = db.users.find_one({"auth0Sub": user.get("sub")}, {"_id": 1})
    caller_points = None
    if caller:
        caller_points = s_new if caller["_id"] == student["_id"] else t_new
