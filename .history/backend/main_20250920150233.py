...
async def complete_request(rid: str, user=Depends(auth_user)):
    ...
    if int(student.get("points", 0)) < pts:
    ...
    student_new = db.users.find_one({"_id": student["_id"]}, {"points": 1})
    tutor_new   = db.users.find_one({"_id": tutor["_id"]},   {"points": 1})

    # Identify caller for convenience so UI can update instantly
    caller = db.users.find_one({"auth0Sub": user.get("sub")}, {"_id": 1})
    caller_points = None
    if caller:
        caller_points = student_new["points"] if caller["_id"] == student["_id"] else tutor_new["points"]

    return {
        "ok": True,
        "studentPoints": student_new["points"],
        "tutorPoints": tutor_new["points"],
