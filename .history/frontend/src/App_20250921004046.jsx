...
export default function App() {
  ...
  // view modes:
  // openView: 'others' | 'mine'  (others = show open requests not created by me)
  // acceptedView: 'acceptedByMe' | 'mine' (acceptedByMe = requests I accepted)
  const [openView, setOpenView] = useState("others");
  const [acceptedView, setAcceptedView] = useState("acceptedByMe");
  ...
  async function refetchAll({ openViewOverride, acceptedViewOverride } = {}) {
    const meData = await fetchMe(getAccessTokenSilently);
    // determine open view and accepted view current modes
    const oView = openViewOverride || openView;
    const aView = acceptedViewOverride || acceptedView;

    // OPEN
    let openL = [];
    if (oView === "mine") {
      openL = await fetchByStatus("open", true);
    } else {
      // show others: fetch all open then filter out requests created by me
      const allOpen = await fetchByStatus("open", false);
      openL = allOpen.filter((r) => r.studentId !== meData._id);
    }

    // ACCEPTED
    let accL = [];
    // acceptedView dictates whether we show requests the user created, or requests the user accepted
    // fetch mine=1 to get requests where user is student or tutor, then filter
    const acceptedMine = await fetchByStatus("accepted", true);
    if (aView === "mine") {
      accL = acceptedMine.filter((r) => r.studentId === meData._id);
    } else {
      accL = acceptedMine.filter((r) => r.tutorId === meData._id);
    }

    // COMPLETED - always show both (requests where user is student or tutor)
    const compL = await fetchByStatus("completed", true);
  ...
  // keep lists in sync when tab or mineOnly changes
  useEffect(() => {
    if (!isAuthenticated || isLoading) return;
    void refetchAll();
          ...
          element={
            <Requests
              ...
              onComplete={handleComplete}
                ...
                onMineToggle={(v) => {
                  setMineOnly(v);
                  // immediately refetch using the new filter value
                  void refetchAll(v);
                }}
              disabled={busy}
            />
