...
export default function App() {
  ...
  async function refetchAll(mineOverride) {
    const useMine = typeof mineOverride === "boolean" ? mineOverride : mineOnly;
    const [meData, openL, accL, compL] = await Promise.all([
      fetchMe(getAccessTokenSilently),
      fetchByStatus("open", useMine),
      fetchByStatus("accepted", useMine),
      fetchByStatus("completed", useMine),
    ]);
    setMe(meData);
    setOpen(openL);
    setAccepted(accL);
