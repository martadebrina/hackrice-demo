...
const myTz = Intl.DateTimeFormat().resolvedOptions().timeZone;
  ...
  items={accepted}
  renderExtra={(item) => {
    const mineIsStudent = me && item.studentId === me._id;
    ...
    return (
      <div style={{ marginTop: 8, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {/* Actions */}
        {(!s || s === "unset" || s === "cancelled") && (
          <ScheduleForm
            disabled={busy}
            onSubmit={async ({ dtLocal, duration }) => {
              await scheduleRequest(getAccessTokenSilently, item._id, {
        ...
        {s === "proposed" && !amProposer && (
          <>
            <button disabled={busy} onClick={async () => { await confirmSchedule(getAccessTokenSilently, item._id); }}>
              Confirm
            </button>
            <button disabled={busy} onClick={async () => { await cancelSchedule(getAccessTokenSilently, item._id); }}>
              Cancel
            </button>
          </>
        )}
