...
// src/lib/api.js
export async function updateProfile(getToken, { name, avatarUrl }) {
  const token = await getToken({ audience: import.meta.env.VITE_AUTH0_AUDIENCE });
...
export async function listSchedules(getToken, rid) {
  const token = await getToken({ audience: import.meta.env.VITE_AUTH0_AUDIENCE });
  const res = await fetch(`${import.meta.env.VITE_API_URL}/requests/${rid}/schedules`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error("List schedules failed");
  return res.json();
}

export async function proposeSchedule(getToken, rid, { start, end, note }) {
  const token = await getToken({ audience: import.meta.env.VITE_AUTH0_AUDIENCE });
  const res = await fetch(`${import.meta.env.VITE_API_URL}/requests/${rid}/schedules`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  if (!res.ok) throw new Error("Propose schedule failed");
  return res.json();
}

export async function decideSchedule(getToken, rid, sid, action /* "accept"|"decline" */) {
  const token = await getToken({ audience: import.meta.env.VITE_AUTH0_AUDIENCE });
  const res = await fetch(`${import.meta.env.VITE_API_URL}/requests/${rid}/schedules/${sid}`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    body: JSON.stringify({ action }),
  });
  if (!res.ok) throw new Error("Decide schedule failed");
  return res.json();
}
