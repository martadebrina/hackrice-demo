...
export default function App() {
  ...
  // Prefer a friendly name from the backend (`me`) first, then the Auth0 `user.name`.
  // If those contain an email address (some providers set `name` to the email),
  // ignore them and fall back to `user.nickname` or the email local-part.
  const looksLikeEmail = (s) => typeof s === "string" && s.includes("@");
  const displayName =
    (me?.name && !looksLikeEmail(me.name) && me.name) ||
