...
export default function RequestCard({ r, onAccept, onComplete, disabled }) {
  return (
    <div style={row}>
      <div>
      ...
      <div>
        {r.status === "open" && (
          <button style={btn} onClick={() => onAccept(r._id)} disabled={disabled}>Accept</button>
        )}
        {r.status === "accepted" && (
          <button style={btn} onClick={() => onComplete(r._id)} disabled={disabled}>Complete</button>
        )}
      </div>
    </div>
  );
