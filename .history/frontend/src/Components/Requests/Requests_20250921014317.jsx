...
// inside RequestCard (Requests.jsx)
function SchedulePanel({ r, onPropose, onDecide, meId }) {
  const [start, setStart] = React.useState("");
  const [end, setEnd]     = React.useState("");
  const [note, setNote]   = React.useState("");

  const canSchedule = ["open", "accepted"].includes(r.status) &&
                      (r.studentId === meId || r.tutorId === meId);
  ...
  return (
    <div className="sched">
      ...
      <div className="sched-row">
        ...
        <button className="btn small" onClick={() => onPropose({ start: new Date(start).toISOString(), end: new Date(end).toISOString(), note })} disabled={!start||!end}>
          Propose
        </button>
      </div>
            ...
            <div key={s._id} className={`sched-item ${s.status}`}>
              ...
              {s.status === "proposed" && s.proposerId !== meId && (
                <div className="sched-actions">
                  <button className="btn small btn-accept" onClick={()=>onDecide(s._id, "accept")}>Accept</button>
                  <button className="btn small btn-ghost"  onClick={()=>onDecide(s._id, "decline")}>Decline</button>
                </div>
              )}
            </div>
          ))}
