      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        {tab === "open" && (
          <RequestsSection
            ...
            onSchedule={async (id) => {
              try {
                const when = prompt(
                  "Enter scheduled start in ISO format (e.g. 2025-09-20T15:00:00):",
                );
                if (!when) return;
                const duration = parseInt(prompt("Duration in minutes (e.g. 60):"), 10) || 60;
                setBusy(true);
                await scheduleRequest(getAccessTokenSilently, id, {
                  scheduledAtISO: when,
                ...
                console.error(e);
                alert("Scheduling failed: " + (e.message || e.status));
              } finally {
                setBusy(false);
              }
            }}
        ...
        {tab === "accepted" && (
          <RequestsSection
            ...
            onConfirmSchedule={async (id) => {
              try {
                setBusy(true);
                await confirmSchedule(getAccessTokenSilently, id);
                await refetchAll();
                alert("Schedule confirmed");
              } catch (e) {
                console.error(e);
                alert("Confirm failed: " + (e.message || e.status));
              } finally {
                setBusy(false);
              }
            }}
            onCancelSchedule={async (id) => {
              try {
                setBusy(true);
                await cancelSchedule(getAccessTokenSilently, id);
                await refetchAll();
                alert("Schedule canceled");
              } catch (e) {
                console.error(e);
                alert("Cancel failed: " + (e.message || e.status));
              } finally {
                setBusy(false);
              }
            }}
