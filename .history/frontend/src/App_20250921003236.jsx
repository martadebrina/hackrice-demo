...
export default function App() {
  ...
  async function refetchAll() {
    const [meData, openL, accL, compL] = await Promise.all([
      ...
      fetchByStatus("open", mineOnly),
      fetchByStatus("accepted", mineOnly),
      fetchByStatus("completed", mineOnly),
    ]);
    setMe(meData);
    setOpen(openL);
    setAccepted(accL);
          ...
          element={
            <Requests
              ...
              onComplete={handleComplete}
                ...
                onMineToggle={(v) => {
                  setMineOnly(v);
                  // trigger a refresh using the new filter
                  setTimeout(() => refetchAll(), 10);
                }}
              disabled={busy}
            />
          }
